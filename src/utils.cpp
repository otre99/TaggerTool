...
QMap<QString, QList<QRectF> > Helper::ReadCSVSingleFile(const QString &path)
{
    ...
    while ( !in.atEnd() ) {
        ...
        if (items.isEmpty() || items.size() < 2) break;
...
void Helper::SaveCSVSingleFile(const QString &path, const QMap<QString, QList<QRectF>> &bboxes, const QString &img_id, const QSize &img_size)
{
    QFile file(path); file.open(QIODevice::WriteOnly);
    QTextStream in(&file);
    for (const auto &key : bboxes.keys()){
...
void Helper::SaveXMLSingleFile(const QString &path, const QMap<QString, QList<QRectF>>& bboxes, const QString &img_id, const QSize &img_size)
{
    QFile xmlfile(path);
    ...
    for (const auto &label : labels){
        for (const auto &box : bboxes[label]){
            ...
            xmlstream.writeTextElement("xmin", QString::number(static_cast<int>(box.left())));
            xmlstream.writeTextElement("ymin", QString::number(static_cast<int>(box.top())));
            xmlstream.writeTextElement("xmax", QString::number(static_cast<int>(box.right())));
            xmlstream.writeTextElement("ymax", QString::number(static_cast<int>(box.bottom())));
